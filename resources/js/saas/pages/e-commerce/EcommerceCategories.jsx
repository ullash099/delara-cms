...
export default function EcommerceCategories(props) {
        ...
        .then(function (response) {
            ...
            if(Object.keys(info.categories).length > 0){
                Object.values(info.categories).map(category=>{
                    let name = props.lang == `en` ? category.name : category.name_l ? category.name_l : category.name
                    let slug = props.lang == `en` ? category.slug : category.slug_l ? category.slug_l : category.slug
    ...
    const handleSearchTable = async (e) => {
        ...
        setRefreshingList(true)
        await axios.get(SecureApiUrl(`get-ecommerce-categories?src=${srcKey}`),{ headers: Header() })
        .then(function(response){
            let info = response.data
            if(info.deny){
                navigate(-1)
    ...
    const handleSavingInfo = async () => {
        ...
        await axios.post(SecureApiUrl(`save-ecommerce-category`),form,{headers:Header()})
        .then(function (response) {
            setSavingData(false)
            let info = response.data
            if(info.deny){
    ...
    const handleBlock = async (id) => {
        ...
        .then(async (res) => {
            if (res.isConfirmed) {
                ...
                const form = new FormData()
                form.append('id', id)
                await axios.post(SecureApiUrl(`block-ecommerce-category`),form,{headers : Header()})
                .then(function (response) {
                    handleGetStartUpData()
                    let info = response.data
                    if(info.deny){
    ...
    const handleUnblock = async (id) => {
        ...
        .then(async (res) => {
            if (res.isConfirmed) {
                ...
                const form = new FormData()
                form.append('id', id)
                await axios.post(SecureApiUrl(`unblock-ecommerce-category`),form,{headers : Header()})
                .then(function (response) {
                    handleGetStartUpData()
                    let info = response.data
                    if(info.deny){
                                                    ...
                                                    Object.values(datatable.infos).map((info,index)=>(
                                                        <tr key={index}>
                                                            <td>
                                                                ...
                                                                <p className='m-0'>
                                                                    {
                                                                        props.lang == `en` ? info.slug : 
                                                                        info.slug_l ? info.slug_l : info.slug
                                                                    }
                                                                </p>
                                                                {info.parent_id ? (
                                                                    <Badge>
                                                                        {
                                                                            props.lang == `en` ? info.parent.name : 
                                                                            info.parent.name_l ? info.parent.name_l : info.parent.name
                                                                        }
                                                                    </Badge>
                                                                ) : (``)}
                                                            </td>
